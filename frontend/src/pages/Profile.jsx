...
export default function Profile() {
    ...
    const handleSendOTP = async () => {
        try {
            const res = await axios.post("http://localhost:5000/api/auth/sendotp", { email: passwords.email });
            if (res.data.success) { setOtpSent(true); toast.success("OTP sent to your email!"); }
        } catch (err) {
    ...
    const handleChangePassword = async () => {
        if (!passwords.otp || !passwords.password || !passwords.confirm) { toast.error("Please fill all fields!"); return; }
        ...
        try {
            ...
            const res = await axios.post("http://localhost:5000/api/auth/resetpassword", {
                email: passwords.email, otp: passwords.otp, password: passwords.password,
            });
            if (res.data.success) {
                            ...
                            {tabs.map((tab) => (
                                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                                    className={`flex items-center justify-between px-5 py-4 rounded-2xl font-bold text-sm uppercase tracking-[1px] transition ${
                                        activeTab === tab.id ? "bg-[#8da27f] text-white" : "text-gray-400 hover:text-white hover:bg-white/5"
                                    }`}>
                            ...
                            <div className="bg-[#161616] rounded-[32px] border border-white/10 p-8">
                                ...
                                <div className="flex flex-col gap-5">
                                    {[
                                        ...
                                        <div key={label} className="flex items-center gap-5 bg-black rounded-2xl border border-white/10 px-6 py-5">
                                            <div className="w-10 h-10 rounded-full bg-[#8da27f]/10 flex items-center justify-center text-[#8da27f] shrink-0">{icon}</div>
                                            <div>
                                ...
                                <div className="mt-8 p-5 bg-black rounded-2xl border border-[#8da27f]/20 flex items-center gap-4">
                                    ...
                                    <p className="text-gray-400 text-sm leading-relaxed">Your account is verified and secure.</p>
                                </div>
                            </div>
                        )}
                                                    ...
                                                    <div className="flex flex-col gap-3 mb-4">
                                                        {items.map((item, i) => (
                                                            <div key={i} className="flex items-center gap-4">
                                                                ...
                                                                <p className="text-[#8da27f] font-black text-sm">Rs.{Number(item.price).toLocaleString()}</p>
                                                            </div>
                                                        ))}
                                                    </div>
                            ...
                            <div className="bg-[#161616] rounded-[32px] border border-white/10 p-8">
                                ...
                                {wishlist.length === 0 ? (
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        {wishlist.map((item) => (
                                            <div key={item.id} onClick={() => navigate(`/product/${item._id || item.id}`)}
                            ...
                            <div className="bg-[#161616] rounded-[32px] border border-white/10 p-8">
                                ...
                                <div className="flex flex-col gap-5 max-w-lg">
                                    <div className="flex flex-col gap-2">
                                        <label className="text-gray-400 text-xs uppercase tracking-[2px] font-bold flex items-center gap-2"><Mail size={12} /> Email</label>
                                        <input value={passwords.email} readOnly
                                    ...
                                    {!otpSent ? (
                                        ...
                                        <>
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <input value={passwords.otp} onChange={(e) => setPasswords({ ...passwords, otp: e.target.value })}
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <label className="text-gray-400 text-xs uppercase tracking-[2px] font-bold flex items-center gap-2"><Lock size={12} /> New Password</label>
                                                <div className="relative">
                                                    ...
                                                    <button onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white transition">
                                                        {showPass ? <EyeOff size={16} /> : <Eye size={16} />}
                                                    </button>
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-2">
                                                <label className="text-gray-400 text-xs uppercase tracking-[2px] font-bold">Confirm Password</label>
                                                <div className="relative">
                                                    <input type={showConfirm ? "text" : "password"} value={passwords.confirm}
                                                        onChange={(e) => setPasswords({ ...passwords, confirm: e.target.value })}
                                                        placeholder="Confirm new password"
                                                        className="w-full bg-black border border-white/10 rounded-2xl px-5 h-12 text-white text-sm outline-none focus:border-[#8da27f] transition placeholder:text-gray-600 pr-12" />
                                                    <button onClick={() => setShowConfirm(!showConfirm)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white transition">
                                            ...
                                            <div className="flex gap-3">
                                                ...
                                                <button onClick={() => { setOtpSent(false); setPasswords({ ...passwords, otp: "", password: "", confirm: "" }); }}
