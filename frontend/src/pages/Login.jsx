...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // ── Save everything needed across the app ──────────────
    ...
    <section className="min-h-screen bg-[#f3f3f3] flex overflow-hidden">
      ...
      <div className="hidden lg:flex flex-1 bg-black relative items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-6 left-6 z-20 flex items-center gap-3">
          <img src={logo} alt="" className="w-[48px] h-[48px] rounded-full object-cover border border-white/20 shadow-lg" />
          <h1 className="text-white text-[30px] font-black tracking-[-2px]">Brand_Shoe</h1>
        </div>
        ...
        <div className="w-full max-w-[380px]">
          ...
          <div className="mt-10">
            ...
            <button
              onClick={handleLogin}
              disabled={loading}
              className="w-full h-[58px] bg-black text-white rounded-[14px] mt-8 text-[13px] tracking-[3px] font-bold hover:bg-[#6f8f62] transition duration-300 shadow-xl disabled:opacity-60 disabled:cursor-not-allowed"
            >
