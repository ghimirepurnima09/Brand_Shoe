...
// Helper: get the best display image from a product
...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // ✅ Uses /newarrivals — returns products where is_new = true (up to 20)
        const res = await axios.get("http://localhost:5000/api/products/newarrivals");
        if (res.data.success) {
    ...
    <section className="bg-black min-h-screen pb-20">
      ...
      <div className="relative h-[520px] overflow-hidden rounded-b-[50px]">
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-6 lg:px-16 pt-[82px]">
          <p className="text-[#8da27f] tracking-[4px] text-[12px] font-bold">BRAND_SHOE PREMIUM COLLECTION</p>
          <h1 className="text-white text-[65px] lg:text-[95px] leading-[88%] font-black tracking-[-6px] mt-5">NEW<br />ARRIVALS</h1>
          <p className="text-gray-200 text-[16px] leading-[32px] mt-7 max-w-[720px]">Fresh drops from the world's most iconic sneaker brands.</p>
        </div>
      </div>
      ...
      <div className="flex gap-10 px-6 lg:px-16 mt-8">
        ...
        <div className="flex-1">
          {loading ? (
            <div className="flex justify-center items-center h-[300px]"><h1 className="text-white text-2xl font-bold">Loading New Arrivals...</h1></div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[300px] gap-4">
              <p className="text-gray-400 text-xl font-bold">No new arrivals match your filters</p>
              ...
              {filtered.map((product) => (
                <div key={product.id} className="bg-[#161616] rounded-[32px] overflow-hidden border border-white/10 hover:-translate-y-2 transition duration-500">
                  <div className="relative h-[280px] overflow-hidden">
                    ...
                    {/* NEW badge */}
                    <div className="absolute top-4 left-4 bg-[#8da27f] text-white px-3 py-1 rounded-full text-[10px] font-bold tracking-[2px]">
                      NEW
                    </div>
