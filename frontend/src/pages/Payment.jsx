// pages/Payment.jsx  — key changes: saves order to DB on success, image fix
...
// ── Saves order to DB, then calls onSuccess ──
async function saveOrder({ form, cart, totalPrice, paymentMethod }) {
  ...
  await axios.post(`${BASE}/api/orders/place`, {
    ...
    items:      cart.map((item) => ({
      id:       item.id,
      name:     item.name,
      // ✅ FIX: store just the path (/uploads/xxx.jpg), not the full URL
      image:    item.image?.startsWith("http") ? new URL(item.image).pathname : item.image,
      price:    item.price,
      size:     item.size,
...
function CheckoutForm({ form, handleChange, cart, totalPrice, paymentMethod, setPaymentMethod, onSuccess, navigate }) {
  ...
  // ✅ FIX: resolve correct image src for cart items
  const imgSrc = (item) => {
    if (!item.image) return "https://via.placeholder.com/68x68?text=Shoe";
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-10 items-start">
        ...
        <div className="bg-[#161616] rounded-[32px] border border-white/10 p-7 sticky top-[120px]">
            ...
            {cart.map((item) => (
              <div key={`${item.id}-${item.size}`} className="flex items-center gap-4">
                <div className="w-[68px] h-[68px] rounded-2xl bg-black border border-white/10 overflow-hidden shrink-0">
                  {/* ✅ FIX: resolve image URL correctly */}
                  <img src={imgSrc(item)} alt={item.name} className="w-full h-full object-cover"
                    onError={(e) => { e.target.src = "https://via.placeholder.com/68x68?text=Shoe"; }} />
                </div>
          ...
          <div className="flex flex-col gap-3 text-sm mb-5">
            {[["Subtotal", `Rs.${totalPrice.toLocaleString()}`],["Shipping (Expedited)", `Rs.${shipping.toLocaleString()}`],["Authentication Fee","FREE"],["Estimated Tax", `Rs.${tax.toLocaleString()}`]].map(([label, val]) => (
          ...
          <PayButton form={form} cart={cart} totalFinal={grandTotal} totalPrice={totalPrice} paymentMethod={paymentMethod} onSuccess={onSuccess} />
...
export default function Payment() {
  ...
  const [form, setForm] = useState({ firstName:"", lastName:"", email:"", address:"", city:"", state:"", zip:"" });
  ...
  return (
    <>
      ...
      <Elements stripe={stripePromise}>
        <CheckoutForm form={form} handleChange={handleChange} cart={cart} totalPrice={totalPrice}
          paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod}
          onSuccess={handleSuccess} navigate={navigate} />
      </Elements>
    </>
  );
}