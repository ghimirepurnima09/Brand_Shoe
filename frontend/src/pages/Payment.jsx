        ...
        return (
          <div key={label} className="flex items-center">
            <div className="flex flex-col items-center">
              ...
              <span className={`text-xs mt-1.5 font-semibold tracking-widest uppercase ${active ? "text-[#8da27f]" : "text-gray-600"}`}>
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
...
function PayButton({ form, cart, totalFinal, totalPrice, paymentMethod, onSuccess }) {
  ...
  const validate = () => {
    ...
    if (cart.length === 0) { toast.error("No items to order!"); return false; }
    return true;
  };
...
function CheckoutForm({
  form, handleChange, cart, totalPrice,
  paymentMethod, setPaymentMethod, onSuccess, navigate, isBuyNow,
}) {
  return (
    <section className="min-h-screen bg-black px-6 lg:px-16 pt-[120px] pb-20">
      {/* Back button — Buy Now goes back to previous page, cart goes to /cart */}
      <button
        onClick={() => isBuyNow ? navigate(-1) : navigate("/cart")}
        className="flex items-center gap-2 text-[#8da27f] font-bold uppercase tracking-[3px] text-sm mb-10 hover:text-white transition"
      >
        <ArrowLeft size={18} /> {isBuyNow ? "Back" : "Back to Cart"}
      </button>
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-10 items-start">
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="bg-[#161616] rounded-[32px] border border-white/10 p-8">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {[
                ["card", "Credit / Debit Card", <CreditCard size={17} />],
                ["cod",  "Cash on Delivery",    <Banknote   size={17} />],
              ].map(([val, label, icon]) => (
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#635bff] flex items-center justify-center shrink-0">
                    <svg width="20" height="20" viewBox="0 0 100 100">
                      <text x="50" y="68" textAnchor="middle" fontSize="62" fontWeight="bold" fill="white" fontFamily="Arial">S</text>
                    </svg>
                  </div>
                  <div>
        ...
        <div className="bg-[#161616] rounded-[32px] border border-white/10 p-7 sticky top-[120px]">
          ...
          {/* Buy Now badge */}
          {isBuyNow && (
            <div className="mb-4 px-3 py-2 bg-[#8da27f]/10 border border-[#8da27f]/30 rounded-2xl">
              <p className="text-[#8da27f] text-xs font-black uppercase tracking-[2px]">⚡ Express Buy Now</p>
            </div>
          )}
          ...
          <div className="flex justify-between items-baseline mb-1">
            ...
            <span className="text-white font-black text-2xl tracking-tight">Rs.{totalPrice.toLocaleString()}</span>
          </div>
...
export default function Payment() {
  ...
  // ── Buy Now detection ─────────────────────────────────────────
  // If navigated from Buy Now, use the state items instead of cart
  const buyNowState = location.state?.buyNow ? location.state : null;
  const activeCart  = buyNowState ? buyNowState.items      : cart;
  const activeTotal = buyNowState ? buyNowState.total      : totalPrice;
