  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { name, email, password, phone }   // ← phone now included
      );
        ...
        <div className="w-full max-w-[1280px] grid lg:grid-cols-2 gap-8 items-center">
          ...
          <div className="px-1 lg:px-8">
            ...
            <div className="mt-9 space-y-5">
              ...
              {/* PHONE */}
              <div>
                <label className="text-[12px] font-bold tracking-[2px] text-gray-500">PHONE NUMBER</label>
                <div className={`mt-2 h-[58px] rounded-[16px] border bg-white px-4 flex items-center gap-3 shadow-sm ${
                  phone.length === 10 ? "border-green-400" : "border-gray-300"
                }`}>
                  ...
                  <input
                    ...
                    className="w-full outline-none bg-transparent text-[15px]"
                  />
                  <span className={`text-[12px] shrink-0 ${phone.length === 10 ? "text-green-500" : "text-gray-400"}`}>
                    {phone.length}/10
                  </span>
                </div>
                <p className="text-[11px] text-gray-400 mt-1 ml-1">
                  This number will be used as your eSewa ID
                </p>
              </div>
              ...
              <p className="text-center text-gray-600 text-[15px]">
                ...
                <Link to="/login" className="text-black font-semibold ml-2 hover:text-[#6f8f62] transition">
                  Log in
                </Link>
              </p>
