...
import { Heart, ShoppingCart, Sparkles, Flame, Tag, ArrowLeft, SlidersHorizontal, X, ChevronDown, ChevronUp } from "lucide-react";
...
export default function MostSoldPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // ✅ Uses /mostsold — returns products where is_most_sold = true (up to 20)
        const res = await axios.get("http://localhost:5000/api/products/mostsold");
        if (res.data.success) {
  ...
  return (
    <section className="bg-black min-h-screen pb-20">
      ...
      <div className="relative h-[520px] overflow-hidden rounded-b-[50px]">
        <img src="https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=1800&auto=format&fit=crop" alt="" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-6 lg:px-16 pt-[82px]">
          <p className="text-[#8da27f] tracking-[4px] text-[12px] font-bold">BRAND_SHOE BESTSELLERS</p>
          <h1 className="text-white text-[65px] lg:text-[95px] leading-[88%] font-black tracking-[-6px] mt-5">MOST<br />SOLD</h1>
          <p className="text-gray-200 text-[16px] leading-[32px] mt-7 max-w-[720px]">The shoes everyone is wearing. Trusted, loved, and flying off the shelves.</p>
        </div>
      </div>
      ...
      <div className="flex gap-10 px-6 lg:px-16 mt-8">
        ...
        <div className="flex-1">
          {loading ? (
            <div className="flex justify-center items-center h-[300px]"><h1 className="text-white text-2xl font-bold">Loading Most Sold...</h1></div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[300px] gap-4">
              <p className="text-gray-400 text-xl font-bold">No best sellers match your filters</p>
              ...
              {filtered.map((product, index) => (
                <div key={product.id} className="bg-[#161616] rounded-[32px] overflow-hidden border border-white/10 hover:-translate-y-2 transition duration-500">
                  <div className="relative h-[280px] overflow-hidden">
                    ...
                    {/* RANK BADGE */}
                    <div className="absolute top-4 left-4 w-9 h-9 rounded-full bg-orange-500 text-white text-sm font-black flex items-center justify-center shadow-lg">
                      #{index + 1}
                    </div>
                    <button
